...
const BirdDetail: React.FC = () => {
  ...
  const getFilteredBirdList = () => {
    const savedSearchParams =
      sessionStorage.getItem("checklistSearchParams") || "";
    ...
    const filters = {
      ...
      excludeOccasionalVisitors:
        params.get("excludeOccasionalVisitors") === "true",
    };
    ...
    let filteredBirds = uruguayBirds.filter((b: any) => {
      ...
      if (
        ...
        b.conservationStatus !== filters.conservationStatus
      )
        return false;
  ...
  const previousBird =
    currentIndex > 0 ? filteredBirdList[currentIndex - 1] : null;
  const nextBird =
    currentIndex < filteredBirdList.length - 1
      ? filteredBirdList[currentIndex + 1]
      : null;
  ...
  const handleRemoveObservation = (observationId: string) => {
    if (
      window.confirm("¿Estás seguro de que quieres eliminar esta observación?")
    ) {
    ...
    <Box>
      ...
      >
        ...
        <Box sx={{ display: "flex", gap: 1, ml: "auto" }}>
          <Tooltip
            title={
              previousBird
                ? `Anterior: ${previousBird.commonName}`
                : "No hay ave anterior"
            }
          >
            <span>
          ...
          <Tooltip
            title={
              nextBird
                ? `Siguiente: ${nextBird.commonName}`
                : "No hay ave siguiente"
            }
          >
            <span>
              <IconButton
      ...
      <Card sx={{ mb: 3 }}>
        <Box sx={{ position: "relative" }}>
          <Box
            sx={{ height: bird.cardHeight || { xs: 710, lg: 760, xl: 760 } }}
          >
          ...
          >
            <Tooltip
              title={
                observation?.seen ? "Marcar como no visto" : "Marcar como visto"
              }
            >
              <IconButton
                size="small"
          ...
          >
            ...
            >
              <Box>
                ...
                >
                  <Typography variant="h4">{bird.commonName}</Typography>
                  <Typography
                    variant="h6"
              ...
              <Box sx={{ display: "flex", gap: 1, flexWrap: "wrap" }}>
                <Chip
                  ...
                  onClick={() =>
                    handleChipClick(
                      "commonness",
                      getCommonnessForDepartment(bird)
                    )
                  }
                  sx={clickableChipStyle}
                />
          ...
          <Paper sx={{ p: 2, mb: 3 }}>
            ...
            {departamentos.length > 0 ? (
              <Box sx={{ display: "flex", flexWrap: "wrap", gap: 1 }}>
                {departamentos.map((departamento: string) => (
                  <Chip
                    ...
                    variant="outlined"
                    onClick={() =>
                      handleChipClick("departamento", departamento)
                    }
                    sx={clickableChipStyle}
                  />
                ))}
              ...
              <Typography
                ...
                sx={{ fontStyle: "italic" }}
              >
                No hay información especificada sobre la abundancia por
                departamentos para esta ave.
              </Typography>
            )}
          </Paper>
        );
        ...
        <Grid item xs={12} md={6}>
          <Paper sx={{ p: 2 }}>
            ...
            <Typography variant="body2" paragraph>
              {bird.description?.general || "Descripción no especificada."}
            </Typography>
          </Paper>
        </Grid>
